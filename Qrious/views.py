...
def detail(request, pk):
    ...
    if up.question_no == product.question_no:
        ...
        leaderboard = {
                Player.objects.order_by('score').reverse()
                          }
        i = 0
        ...
        skips = 3 - s
        return render(request, 'Qrious/detail.html',
                      {'product': product, 'player': up, 'skips': skips, 'leaderboard': leaderboard, 'error': error})
    else:
